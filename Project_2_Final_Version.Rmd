...
# Introduction and Project 1 Recap

Project 2 aims at analyzing and make predictions on the Olympic Games dataset. We decided to keep the same dataset of Project 1 because we thought that it was worth analyzing and exploring this dataset more in depth, and that the analysis performed for Project 1 left several interesting questions unanswered. As a recap, the Olympic Games dataset is a historical dataset that contains data from Athens 1886 to Rio 2016. The dataset was found on Kaggle and originally scrapped from sports-reference.com in May 2018. Our dataset consists of 27116 observations and 15 variables/columns that are ID (unique number of each athlete), Name (athlete's name), Sex (M or F), Age (Integer), Weight (in kilograms), Height (in centimeters), Team (Team name), NOC (National Olympic Committee 3-letter code), Games (year and season), Year (integer), Season (Summer or Winter), City (Host city), Sport, Event,  Medal (gold, silver, bronze, or NA). The follwing is the structure of the original dataset: 
...
In addition, in order to make our analysis more interesting,  we decided to add the Gross Domestic Product (GDP) of each country (when available) to our data frame. We retrieved our data on GDP from the GDP dataset available on the World Bank website. This dataset contains most of the GDPs for most countries in the world from 1960 to 2018. The dataset is rather complete, although it contain several NAs, especially for small and poor countries. 

In this project, we used different types of inferential statistic, such as ANOVA test and t-test, to analyze our data and infer insightful conclusions. Moreover, we used various supervised and unsupervised machine learning techniques to make predictions and discover, visualize, and explore patterns in the Olympics dataset. The project focuses on three main SMART questions: 

1.	SMART Question 1: Does the hosting team have an advantage? 

2.	SMART Question 2: Does height, age, and weight determine if an Olympic athlete wins a medal?

3.	SMART Question 3: How does GDP determine statistics in the Olympics? Is GDP a good predictor?

## Descriptive Statistics

Before diving into the analysis, we included some graphs from the previous part as a refresher for the reader. The following graphs display the main descriptive statistics that are necessary to understand the dataset and our analysis. 
...
```{r Freq_table_number_of_medal}
# Frequency table of total numebr of medals won by men and women
JustWinners <- summer_w_NAs[!is.na(summer_w_NAs$Medal), ]
JustWinners['Number_of_Medals'] <- 1
GenderFreq <- table(JustWinners$Sex)
GenderFreq <- data.frame(GenderFreq)
GenderFreq
```

```{r Descriptive Stats Medals Graphs_1, echo=FALSE, include=TRUE}
ggplot(GenderFreq, aes(x = Var1, y = Freq, fill = Var1)) + geom_bar(stat = 'identity', width = 0.5) + scale_fill_manual(values=c('F' = 'deeppink1', 'M' = 'dodgerblue1')) + ggtitle('Comparison of Medals Won by Men and Women') + labs(x = 'Sex', y = 'Number of Medals')
```

The first graph is a representation of top countries total medals earned by medal type. This clearly shows teams that have dominated the 29 Olympic games. The second graph displays the number of medals won by men and women. From the graph, it is clear that men won more medals than women, where number of medals won by men more than 15,000 and women less than 10,000. One reason of less medals won by women was that women did not participated Olympic games before 1900.

### Athletes Participation Over 120 Years of Summer Olympic Games

The following graph displays the number of female and male athletes who participated to the Olympics over time: 

```{r Descriptive Statistics Athletes}
# plot male female participants over time
group_count_mf <- table(summer_w_NAs$Sex, summer_w_NAs$Year)
group_count_mf <- data.frame(group_count_mf)
group_count_mf
```

```{r Plotting Partecipants Over Time, echo=FALSE, include=TRUE}
# plot male female participants over time
ggplot(group_count_mf, aes(x=Var2, y=Freq, group=Var1, color=Var1)) +
  geom_point(size=2) +
  geom_line()  +
  scale_color_manual(values=c("deeppink1","dodgerblue1")) +
  labs(y = 'Number of Athletes', x = "Year", title = "Number of male and female Olympic Athletes over time") +
  theme(plot.title = element_text(hjust = 0.5)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

### Participation by Sport

The following graphs show participation by sport: 
...
We can also conclude from our participation distribution by sports graphs that gymnastics and swimming are by far the most popular sports of the Olympics, however, for females the two aforementioned sports categories make up a predominant amount of participation. 

## Histograms of Age, Height, Weight

We also included Histograms to view the distribution of Age, Height, and Weight for all Olympic Athletes. This is useful to determine the distribution.
...
As we can see from the plots, Age and weight are a bit right-skewed, height is very normal. These distributions do not vary greatly when looking at males and females separately.

# SMART QUESTION I:

## Does hosting team have an advantage? Do medals won in years previous affect medals won in future olympics (ANOVA) - Do teams have 'streaks' of wins? Before/after hosting change in mean medals won?

In this model we will use ANOVA to measure whether hosting the Olympics has an effect on the amount of medals won, as well as the average amount of medals won per evernt (mean). This analysis will be preformed on eight countries with two years before the host event and two years after event. We predict the host country will obtian more medals in their host year and also show an increased medal count in the Olympic events after their host event. To confirm this prediction we will be looking for a low and positive correlation between the event before and host year and a medium to high correlation between the hosting year and event after.
...
subvars = c('Team' , 'Year' , 'Name' , 'Event' , 'Medal')

athlete_events_recode = athlete_events_summer_na.wmedals[subvars]
...
### ANOVA Results Summarized

```{r AOV for SMART1}
#change n = 1 to view all ANOVA tables
n = 7
...
Results from the ANOVA test indicate high and slightly positive correlation between 1960 (host) and 1956 (Year before host), as well as a high and slightly positive correlation between 1960 and 1964 (Year after host). This suggests that the means of the before, host and after years are similar and slightly increasing over time. We can conclude that host year does slightly effect medals won and year after given the lower correlations between host year and 1952/1968.

Japan:

Results from the ANOVA test indicate high and slightly positive correlation between 1964 (host) and 1960 (Year before host), as well as a low and slightly positive correlation between 1964 and 1968 (Year after host). This suggest that the means of before and host years are similar and slightly increasing over time, while the means of host and after are dramaticall different and increasing over time. We can conclude that host year does slightly effect medals won and significantly effects the medals won in the year after host.

Mexico:

Results from the ANOVA test indicate high and slightly positive correlation between 1968 (host) and 1964 (Year before host), as well as moderate, but slightly negative correlation between 1968 and 1972 (Year after host). This suggest that the means of before and host years are similar and slightly increasing over time, while the means of host and after are dramaticall different and decreasing over time. We can conclude that host year does slightly effect medals won and significantly (but negatively) effects  the medals won in the year after host. This is an anomoly in our data.

Spain:

Results from the ANOVA test indicate an extremely low and slightly positive correlation between 1992 (host) and 1988 (Year before host), as well as moderate and slightly positive correlation between 1992 and 1996 (Year after host). This suggest that the means of before and host years are drastically different and increasing over time, while the means of host and after are more similar and increasing over time. We can conclude that host year does extremely effect medals won  and significantly effects  the medals won in the year after host as well.

United States:

Results from the ANOVA test indicate an extremely high and slightly positive correlation between 1996 (host) and 1992 (Year before host), as well as high and slightly positive correlation between 1992 and 1996 (Year after host). This suggest that the means of before and host years are significantly similar and increasing over time, as well as the means of host and after years. We can conclude that host year does slightly effect medals won  and also slightly effects the medals won in the year after host.

Australia:

Results from the ANOVA test indicate an extremely high and slightly negative correlation between 2000 (host) and 1996 (Year before host), as well as high and slightly positive correlation between 2000 and 2004 (Year after host). This suggest that the means of before and host years are significantly similar and decreasing over time, while the means of the host and after years are significantly similar, but increasing over time. We can conclude that host year does not effect medals won and the host year also does not effect the medals won in the year after host.This is an anomoly in our data.

Greece:

Results from the ANOVA test indicate an moderate and slightly negative correlation between 2004 (host) and 2000 (Year before host), as well as high and slightly negative correlation between 2004 and 2008 (Year after host). This suggest that the means of before and host years are significantly different and decreasing over time, as well as the means of host and after years (although these means are more similar). We can conclude that host year does not effect medals won and the host year also does not effect the medals won in the year after host. Although one could make an argument that knowing they had a host year coming up Greece was galvanized and therefore won more medals in the year before.

China:

Results from the ANOVA test indicate an low and slightly positive correlation between 2008 (host) and 2004 (Year before host), as well as high and slightly positive correlation between 2008 and 2012 (Year after host). This suggest that the means of before and host years are significantly different and increasing over time and the means of host and after years are significantly similar and increasing over time. We can conclude that host year does significantly effect medals won and the host year slighlty effects the medals won in the year after host. China also shows that their medal winning streak continues for at least two years after their host. Hosting may have galvanized the people or government into a higher Olympic participation.

### Graphic Examples of Medals won from Greece and China

```{r Plots for SMART1, echo=FALSE, include=TRUE}

#change n = 1 to view all graphs
n = 7
...
Greece shows an increase in medal count in the year they host, however, they show a decrease in average medals won per event. We can conclude that Greece increased its participation in their host year, which caused them to win more medals. Greece also does not exibit "host effect" in the subsequent years after hosting.


China:

China shows a dreamatic increase in medals won and average medals won per event in their host year. They also exhibit the "host effect" in subsequent years by doing better or similar to their host year and significantly better than the years prior to hosting - in terms of medals won per event.

# SMART QUESTION II:

## Does age, height, and weight determine whether an athlete wins an olympic medal?
...
```{r AHW Correlations, echo=FALSE, include=TRUE, warning=FALSE}
...
# 3.2: Does the average Age, Height, or Weight different between olympic medalists and non-medalists?
...
```{r Feature Selection Females Plot, include = TRUE, echo=FALSE}
...
```{r Feature Selection M}
# Males PCA
# data with just Age, height weight for males
age_height_weight_M = summer_M[,4:6]
```

```{r Feature Selection Males Plot, include = TRUE, echo=FALSE}
...
### Does Age, Height, Weight differ between Gymnast medalists and non-medalists?
...
```{r weight t-test, echo=FALSE, include=TRUE, message=FALSE, warning = FALSE}
...
#### Weightliftiing Logistic Regressions
...
To make things more interesting and to deepen our analysis, we decided to introduce GDP as a new variable in this project. We would like to see if GDP has an effect on whether an athlete earns a medal, like we did for sex, age, height, and weight in the previous project. Moreover, we want to see whether GDP influences the gender of the athletes, and the overall number of participants per country. Finally, we want to test whether GDP is a good predictor, and in this part of the paper we will build several models with GDP as a variable to analyze that. In summary, this part of the paper will try to answer the following questions: 

* How does GDP determine statistics in the Olympics?

* Is GDP a good predictor?

In order to answer these questions, we merged Summer_with_NAs – the dataset on the Olympics we have used so far in our analysis – with the gdp dataset retrieved from the World Bank website. Prior to doing that, we merged Summer_with_NAs with the NOC dataset – which contains the NOC codes for the countries – by the NOC column. Summer_with_NAs has column called Team which contains the names of the country an athlete comes from, but it also contains names of different teams, such as rowing teams, gymnastic teams, and so on, which is an irrelevant information for our analysis. Therefore, we merged Summer_with_NAs with NOC to have a more accurate columns that contains the country names and we deleted the Team column. Finally, we merged the new dataset with the gdp dataset by country and year. 
...
The first questions we attempt to answer in this part is if GDP influences whether an athlete earns a medal and if GDP can be a good predictor to predict that. To see if GPD has an effect on whether an athlete wins a medal we firstly display the data in a boxplot. 
...
Through this analysis we got a better insight on the Olympic Games dataset and we were able to understand whether the variables contained in the dataset have an effect on whether an athlete earns a medal, on the gender of the athletes, and on the number of participants. Additionally, adding GDP as a variable allowed us to get a more complete picture of the Olympics world. 

SMART Question 1 showed that hosting the Olympics always increases the amount of medal won compared to previous years and that host countries seem to perform better in years after the Olympics they hosted. Yet, some countries do not exhibit common trends. 

Smart Question 2 showed that age, height, and weight do determine winning of Olympic medals; however, it varies by gender and by sport. For female athletes, increases in age, height, weight increase the chances of winning a medal. For male athletes, increases in weight and height increase the chances of winning a medal; however, an increase in age lowers the chances of winning. This shows that occasionally the host-effect doesnt actually motivate the host athletes to perform better. Some teams also dont continue their winning streak in the Olympic games after hosting, we can conclude that the Olympics aren’t important enough in these countries for hosting to have a lasting effect. 

Smart Question 3 showed that GDP seems to have an effect on whether an athlete earns a medal, on the gender of the athletes, and on the number of participants sent by each country to the Olympics. As most of the models we built using GDP as a variable were not good, we can conclude that GDP per se is not a great predictor, but it definitely made our analysis more thorough and complete. We can conclude that GDP does not only influence sectors that are strictly related to it, such as economy, but it also has an influence on sports.  
...
World Bank. GDP Dataset (current US$). Available at: https://data.worldbank.org/indicator/NY.GDP.MKTP.CD [Accessed November 2019]